...
public class CriarProdutoConsumer : BackgroundService
{
    ...
    public CriarProdutoConsumer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<CriarProdutoConsumer> logger)
    {
    ...
    private async Task PostAsync(Produto model)
    {
        _logger.LogInformation($"Consumer > ExecuteAsync > PostAsync > Produto > CRIAR_PRODUTO > Redis...");
        using var scope = _serviceProvider.CreateScope();
        var scopeService = scope.ServiceProvider.GetRequiredService<IProdutoService>();
        await scopeService.Post(model);
    }
}
