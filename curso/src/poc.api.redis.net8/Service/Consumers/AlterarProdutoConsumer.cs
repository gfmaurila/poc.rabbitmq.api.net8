...
public class AlterarProdutoConsumer : BackgroundService
{
    ...
    public AlterarProdutoConsumer(IConfiguration configuration, IServiceProvider serviceProvider, ILogger<AlterarProdutoConsumer> logger)
    {
    ...
    private async Task PutAsync(Produto model)
    {
        _logger.LogInformation($"Consumer > ExecuteAsync > PutAsync > Produto > ALTERAR_PRODUTO > Redis...");
        using var scope = _serviceProvider.CreateScope();
        var scopeService = scope.ServiceProvider.GetRequiredService<IProdutoService>();
        await scopeService.Put(model);
    }
}
